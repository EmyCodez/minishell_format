...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/13 09:57:14 by esimpson          #+#    #+#             */
/*   Updated: 2024/06/13 10:13:08 by esimpson         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*ft_handle_dollar(char *str, size_t *i, int *exit_code,
		t_env **env_list)
{
	size_t	start;
	char	*var;
...
static char	*ft_cmd_pre_expander(char *str, int *exit_code, t_env **env_list)
{
	...
	while (str[i])
	{
		if (str[i] == '\'')
			...
			ret = ft_strjoin_f(ret, ft_handle_dquotes(str, &i, exit_code,
						env_list));
		else if (str[i] == '$')
			ret = ft_strjoin_f(ret, ft_handle_dollar(str, &i, exit_code,
						env_list));
		else
